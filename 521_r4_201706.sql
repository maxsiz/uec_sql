...
AND   T.KIND=17 
and   O.CODE !='0071' --exclude Kirensk
AND TO_CHAR(TRUNC(T.DATE_OF,'MM'), 'DDMMYYYY') = '01062017'
AND T.INS_DATE < TO_DATE('04.07.2017 14:07:51','DD.MM.YYYY HH24:MI:SS') --врем€ "закрыти€" мес€ца
--exclude >30
and T.ID not in (0)
...
AND   T.KIND=17 
AND TO_CHAR(TRUNC(T.DATE_OF,'MM'), 'DDMMYYYY') = '01062017'
AND T.INS_DATE < TO_DATE('05.06.2017 10:04:31','DD.MM.YYYY HH24:MI:SS') --close month
--exclude >30
and T.ID not in (0)
--перевыпущенна€ карта
AND t.card_num  IN ('00')
...
--, T.*
 FROM CPTT.T_DATA T
    ...
    INNER JOIN CPTT.ROUTE R ON T.id_route=R.ID AND T.id_division = R.id_division
 WHERE 
     nvl(T.NEW_CARD_SERIES, T.CARD_SERIES)  in (80,83)  
AND   T.KIND=17 
and   O.CODE ='0071' --Kirensk
AND TO_CHAR(TRUNC(T.DATE_OF,'MM'), 'DDMMYYYY') = '01062017'
AND T.INS_DATE < TO_DATE('04.07.2017 14:07:51','DD.MM.YYYY HH24:MI:SS') --a?aiy "cae?uoey" ianyoa
--exclude >30
and T.ID not in (0)
--exclude STOLLEN CARD
AND t.card_num NOT IN (
...
SELECT T.CARD_NUM, count (*) as kol_t, MIN(T.INS_DATE) as minins, MAX(T.INS_DATE) as maxins
 FROM CPTT.T_DATA T
    ...
    INNER JOIN CPTT.OPERATOR O ON D.ID_OPERATOR=O.ID  and O.ROLE=1
 WHERE 
    nvl(T.NEW_CARD_SERIES, T.CARD_SERIES)  in (67,80,82,83,84) --+67,84  from may
...
AND   T.KIND=17 
AND TO_CHAR(TRUNC(T.DATE_OF,'MM'), 'DDMMYYYY') = '01062017'
--AND T.INS_DATE < TO_DATE('04.05.2017 9:42:05','DD.MM.YYYY HH24:MI:SS') --врем€ "закрыти€" мес€ца
AND t.card_num NOT IN (
'0003024684' --test card
) 
GROUP BY T.CARD_NUM 

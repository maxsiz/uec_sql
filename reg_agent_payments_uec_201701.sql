...
nvl(T.NEW_CARD_SERIES, T.CARD_SERIES)  in (67,82,83) --Серии карт
 and O.CODE !=99  --''
AND   T.KIND in (7,12)
AND T.DATE_TO=TO_DATE('31.01.2017 00:00:00','DD.MM.YYYY HH24:MI:SS')
...
--part 2
SELECT O.CODE, T.CARD_NUM, T.DATE_TO, T.DATE_OF,'3802;3801111847;042520607;'||SUBSTR(T.CARD_CHIP,1,8)||';9643103883'||SUBSTR(T.CARD_NUM,2)||';'||LPAD(nvl(T.NEW_CARD_SERIES, T.CARD_SERIES),4,'0')||';15000'
--DISTINCT T.CARD_NUM
 FROM CPTT.T_DATA T
 INNER JOIN CPTT.DIVISION D ON T.ID_DIVISION=D.ID
 INNER JOIN CPTT.OPERATOR O ON D.ID_OPERATOR=O.ID  and O.ROLE=2
...
nvl(T.NEW_CARD_SERIES, T.CARD_SERIES)  in (67,82,83) 
 and O.CODE !=99
AND   T.KIND in (7,12)
AND T.DATE_TO>=TO_DATE('31.12.2016 00:00:00','DD.MM.YYYY HH24:MI:SS')
AND T.CARD_NUM   in (
'0003014897'--
)
ORDER BY T.CARD_NUM
